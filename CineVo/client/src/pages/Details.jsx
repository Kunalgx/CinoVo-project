...
export default function Details({ type }) {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("cinevo-region-change", onRegionChange);
  }, []);
  ...
  useEffect(() => {
    ...
    movieService
      ...
      .then(([details, credits, providers]) => {
        ...
        setSeasonNumber(
          data.seasons?.find((item) => item.season_number > 0)?.season_number ||
            1,
        );
      ...
      .catch((loadError) => {
        if (active)
          setError(
            loadError.response?.data?.message || "Unable to load details",
          );
      });
    return () => {
      active = false;
  ...
  useEffect(() => {
    let active = true;
    setWatchmode({ loading: true, available: false, sources: [] });
    movieService
      .watchmode(type, id)
      .then((response) => {
        if (active) setWatchmode({ loading: false, ...response.data.data });
      })
      .catch(() => {
        if (active)
          setWatchmode({
            ...
            available: false,
            sources: [],
            message: "Streaming availability is temporarily unavailable",
          });
      });
    return () => {
      active = false;
 ...
 useEffect(() => {
  ...
  movieService
    .streamingAvailability(type, id, region.toLowerCase())
    .then((response) => {
      if (!active) return;
      ...
      setAudio({
        loading: false,
        audioLanguages: Array.isArray(data.audioLanguages)
          ? data.audioLanguages
          : [],
    ...
    .catch(() => {
      if (active) {
        setAudio({
          ...
          verified: false,
          message:
            "Audio-language availability is temporarily unavailable",
        });
      }
    });
  ...
  useEffect(() => {
    ...
    movieService
      ...
      .then((response) => {
        if (active) setSeason(response.data.data);
      })
      .catch((loadError) => {
        if (active)
          setSeasonError(
            loadError.response?.data?.message || "Unable to load episodes",
          );
      });
    return () => {
      active = false;
      ...
      .then(([watched, scheduled]) => {
        ...
        const items = (response) =>
          (Array.isArray(response.data.items)
            ...
            (item) => item.tmdbId === Number(id) && item.mediaType === type,
          );
        const episodes = (response) =>
          items(response).filter((item) => item.episode);
  ...
  const openAction = async (mode, episode = null) => {
    if (!user) {
      navigate("/login");
      return;
    }
    if (
      !episode &&
      movie &&
      (mode === "watched" ? watchedTitle : scheduledTitle)
    ) {
      try {
        if (mode === "watched") {
          await userService.removeWatched(type, id);
          setWatchedTitle(false);
        } else {
          await userService.removeScheduled(type, id);
          setScheduledTitle(false);
        }
      } catch (toggleError) {
        setDateError(
          toggleError.response?.data?.message || "Could not update this title.",
        );
      }
      return;
    }
  ...
  const saveAction = async () => {
    if (!action || !date) {
      setDateError("Choose a date first.");
      return;
    }
    const chosen = new Date(`${date}T00:00:00`);
    if (Number.isNaN(chosen.getTime())) {
      setDateError("Choose a valid date.");
      return;
    }
    if (action.mode === "watched" && date > today()) {
      setDateError("Watched items cannot use a future date.");
      return;
    }
    ...
    try {
      if (episode) {
        if (action.mode === "watched")
          await userService.addWatchedEpisode(payload);
      ...
      setDateError(
        saveError.response?.data?.message || "Could not save this item.",
      );
    }
  };
  ...
  const providerGroups = [
    ...
    [
      "Free / Ads",
      [...(providerRegion?.free || []), ...(providerRegion?.ads || [])],
    ],
  ...
  const providers = providerGroups
    .flatMap(([label, items]) =>
      items.map((provider) => ({ ...provider, availabilityType: label })),
    )
    .filter(
      (provider, index, list) =>
        list.findIndex(
          (item) =>
            ...
            item.availabilityType === provider.availabilityType,
        ) === index,
    );
    ...
    <main className="pb-12">
      ...
      <section className="relative overflow-hidden border-b border-cine-border">
        ...
        <div className="relative mx-auto grid max-w-6xl gap-7 px-5 pb-10 pt-8 sm:grid-cols-[190px_1fr] sm:items-end sm:pt-10 lg:grid-cols-[230px_1fr] lg:gap-10">
          <img
            ...
            alt={title}
            className="mx-auto w-44 rounded-2xl shadow-2xl sm:mx-0 sm:w-full"
          />
          <div className="min-w-0">
            ...
            <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs text-slate-400">
              <span>{formatDate(dateOf(movie))}</span>
              <span>Rating {Number(movie.vote_average || 0).toFixed(1)}</span>
              {type === "tv" && (
                <span>{movie.number_of_seasons || 0} seasons</span>
              )}
            </div>
            <div className="mt-6 flex flex-wrap gap-2">
              <button
                onClick={() => openAction("watched")}
                className="rounded-lg bg-white px-4 py-2.5 text-sm font-black text-black"
              >
                {watchedTitle ? "Unwatch" : "Mark watched"}
              </button>
              <button
                onClick={() => openAction("scheduled")}
                className="rounded-lg border border-white/20 px-4 py-2.5 text-sm font-black"
              >
                {scheduledTitle ? "Unschedule" : "Schedule"}
              </button>
            </div>
            {dateError && !action && (
              <p className="mt-3 text-sm text-red-300">{dateError}</p>
            )}
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto grid max-w-6xl gap-8 px-5 py-9 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div className="min-w-0 space-y-9">
              ...
              {(movie.credits?.cast || []).slice(0, 12).map((person) => (
                ...
                >
                  <img
                    ...
                    alt={person.name}
                    className="h-32 w-24 rounded-xl object-cover transition duration-200 group-hover:scale-[1.03] group-hover:ring-2 group-hover:ring-cine-gold/70"
                  />
                  <p className="cine-surface-link mt-2 truncate text-xs">
                    {person.name}
                  </p>
                </button>
              ))}
          ...
          <section className="rounded-2xl border border-cine-border bg-cine-panel p-4 sm:p-5">
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-cine-accent">
                  Watch providers
                </p>
                <h2 className="mt-1 text-xl font-black">Available on</h2>
              </div>
              <span className="rounded-full border border-cine-border px-3 py-1 text-xs text-slate-400">
                Region: {region}
              </span>
            </div>
            {providers.length ? (
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                {providers.map((provider) => (
                  <div
                    key={`${provider.provider_id}-${provider.availabilityType}`}
                    className="flex min-w-0 items-center gap-3 rounded-xl border border-cine-border bg-[#151a23] p-3"
                  >
                    <img
              ...
              <p className="mt-5 text-sm text-slate-400">
                No providers listed for {region}.
              </p>
            )}
          </section>
          <WatchmodeAvailability availability={watchmode} />
          <AudioAvailability availability={audio} />
          {type === "tv" && (
            <section className="rounded-2xl border border-cine-border bg-cine-panel p-4 sm:p-5">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-cine-accent">
                ...
                <h2 className="mt-1 text-xl font-black">Seasons & episodes</h2>
              </div>
              <div className="mt-5 flex gap-2 overflow-x-auto border-b border-cine-border pb-2">
                {(movie.seasons || [])
                  .filter((item) => item.season_number > 0)
                  .map((item) => (
                    <button
                      key={item.id}
                      onClick={() => setSeasonNumber(item.season_number)}
                      className={`flex-none rounded-lg px-4 py-2 text-sm font-black transition ${seasonNumber === item.season_number ? "bg-cine-gold text-black" : "border border-cine-border bg-cine-card text-slate-300 hover:border-slate-500"}`}
                    >
              ...
              {seasonError && (
                <p className="mt-4 text-sm text-red-300">{seasonError}</p>
              )}
              {!season && !seasonError ? (
        ...
        <aside className="h-fit rounded-2xl border border-cine-border bg-cine-panel p-5 lg:sticky lg:top-24">
          <h2 className="mb-4 text-xl font-black">Details</h2>
          <dl className="space-y-4 text-sm">
            ...
            {movie.status && (
              <div>
                ...
                <dd className="mt-1 text-slate-300">{movie.status}</dd>
              </div>
            )}
          </dl>
        </aside>
      ...
      {similar.length > 0 && (
        <section className="mx-auto max-w-6xl px-5">
          <h2 className="mb-4 text-xl font-black">You may also like</h2>
          <MovieGrid
            items={similar
              .slice(0, 20)
              .map((item) => ({ ...item, media_type: type }))}
            onDetails={(itemId, itemType) => navigate(`/${itemType}/${itemId}`)}
          />
        </section>
      )}
      <Modal
        ...
        title={
          action?.episode
            ? `${action.mode === "watched" ? "Watch" : "Schedule"} episode`
            : action?.mode === "watched"
      ...
      >
        <p className="mb-4 text-sm text-slate-400">
          {action?.episode
            ? `${action.episode.name || "Episode"} · S${String(action.episode.season_number).padStart(2, "0")}E${String(action.episode.episode_number).padStart(2, "0")}`
            : "Choose a date to keep your CineVo timeline accurate."}
        </p>
        <DatePicker
          value={date}
          onChange={(nextDate) => {
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          ...
          >
            Save {action?.mode === "watched" ? "watched" : "scheduled"}
          </button>
        </div>
      </Modal>
    </main>
...
function AudioAvailability({ availability }) {
  const AUDIO_NAMES = {
    hi: "Hindi",
    en: "English",
    ta: "Tamil",
    ...
    return (
      <section className="rounded-2xl border border-cine-border bg-cine-panel p-4 sm:p-5">
        <div className="flex flex-wrap items-center gap-2">
          <h2 className="text-xl font-black">Audio Languages</h2>
        </div>
  ...
  if (!availability?.verified) {
    return (
      <section className="rounded-2xl border border-cine-border bg-cine-panel p-4 sm:p-5">
        <div className="flex flex-wrap items-center gap-2">
          <h2 className="text-xl font-black">Audio Languages</h2>
          <span className="rounded-full border border-cine-border px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-slate-500">
            Not verified
          </span>
        </div>
        <p className="mt-4 text-sm text-slate-400">
          {availability?.message ||
            "Audio-language availability has not been verified yet."}
        </p>
      </section>
    );
  }

  const languages = Array.isArray(availability.audioLanguages)
    ? availability.audioLanguages
    : [];
  const providers = Array.isArray(availability.providers)
      ...
      {languages.length ? (
        <div className="mt-4 flex flex-wrap gap-2">
          {languages.map((language) => (
            <span
              key={language}
              className="rounded-full border border-cine-border bg-[#151a23] px-3 py-1.5 text-xs font-bold text-slate-200"
            >
              {AUDIO_NAMES[language] || language}
            </span>
          ))}
        </div>
      ) : (
        <p className="mt-4 text-sm text-slate-400">
          No verified audio languages listed.
        </p>
      )}
          ...
          <div className="space-y-2">
            {providers.map((provider, index) => (
              <div
                ...
                className="rounded-xl border border-cine-border bg-[#151a23] p-3"
              >
                <p className="text-sm font-bold text-slate-200">
                  {provider.name || "Unknown provider"}
                </p>
                {Array.isArray(provider.audioLanguages) &&
                  provider.audioLanguages.length > 0 && (
                    <p className="mt-1 text-xs text-slate-400">
                      {provider.audioLanguages
                        .map((language) => AUDIO_NAMES[language] || language)
                        .join(", ")}
                    </p>
                  )}
              </div>
...
function WatchmodeAvailability({ availability }) {
  const groups = [
    ...
    label,
    (availability.sources || []).filter((source) => source.type === type),
  ]);
  return (
    <section className="rounded-2xl border border-cine-border bg-cine-panel p-4 sm:p-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-xl font-black">Where to Watch</h2>
        </div>
        <span className="rounded-full border border-cine-border px-3 py-1 text-xs text-slate-400">
          Region: IN
        </span>
      ...
      {availability.loading ? (
        <p className="mt-5 text-sm text-slate-400">
          Checking Indian streaming availability…
        </p>
      ) : availability.available && availability.sources?.length ? (
            ...
            .map(([label, sources]) => (
              <div key={label}>
                ...
                <div className="flex flex-wrap gap-2">
                  {sources.map((source) => (
                    <a
                      key={`${source.name}-${source.type}-${source.web_url}`}
                      href={source.web_url}
                      target="_blank"
                      rel="noreferrer"
                      className="rounded-lg border border-cine-border bg-[#151a23] px-3 py-2 text-sm font-bold transition hover:border-cine-gold"
                    >
                      {source.name}
                    </a>
                  ))}
  ...
  return (
    <article className="grid min-h-[152px] grid-cols-[140px_minmax(0,1fr)] gap-4 rounded-xl border border-cine-border bg-cine-card p-3 sm:grid-cols-[155px_minmax(0,1fr)]">
      ...
      <div className="flex min-w-0 flex-col">
        <div className="min-w-0">
          <p className="text-[10px] font-black uppercase tracking-wider text-cine-accent">
          ...
          <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-slate-400">
            <span>★ {Number(episode.vote_average || 0).toFixed(1)}</span>
        ...
        <div className="mt-auto flex flex-wrap gap-2 pt-3">
          <button
            onClick={() => onToggle("watched", episode)}
            className={`rounded-lg px-2.5 py-1.5 text-[10px] font-black ${watchedItem ? "bg-cine-gold text-black" : "border border-cine-border text-slate-200"}`}
          >
            {watchedItem ? "Mark Unwatched" : "Mark Watched"}
          </button>
          <button
            onClick={() => onToggle("scheduled", episode)}
            className={`rounded-lg px-2.5 py-1.5 text-[10px] font-black ${scheduledItem ? "bg-cine-accent text-white" : "border border-cine-border text-slate-200"}`}
          >
            {scheduledItem ? "Scheduled" : "Schedule"}
          </button>
        </div>
      </div>
    </article>
